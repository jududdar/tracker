        ...
        <table cellspacing=0 style="width:99%" <?php if (!is_numeric($quest)) echo "class=main"; ?>>
              ...
              */
              if (isset($_GET["search"])) {
                  ...
                  if (is_numeric($query)) {
                      $count = mysql_result(mysql_query("SELECT COUNT(ID) FROM $trinitydb.quest_template WHERE ID = " . mysql_real_escape_string($query)), 0);
                      if ($count == 0) {
                      ...
                      $count = mysql_result(mysql_query("SELECT COUNT(ID) FROM $trinitydb.quest_template WHERE LogTitle LIKE \"%" . mysql_real_escape_string($query) . "%\""), 0);
                      if ($count == 0) {
                          echo "<tr><td colspan=4>No entries found</td></tr>";
                          die();
                      } else {
                          echo "<tr><td>Quest ID</td><td>Name</td><td>Reported Status</td></tr>";
                          $sql = mysql_query("SELECT ID, LogTitle FROM $trinitydb.quest_template WHERE LogTitle LIKE \"%" . mysql_real_escape_string($query) . "%\"");
                          while ($row = mysql_fetch_assoc($sql)) {
                              $res2 = mysql_query("SELECT status, dbver FROM $trackerdb.status WHERE quest_id = " . $row["entry"] . " AND dbver>=" . $c_database_version . " GROUP BY status ASC, dbver DESC");
                              $queststatus = "";
                              while ($row2 = mysql_fetch_array($res2))
              ...
              */
              elseif (isset($_GET["problems"])) {
                  ...
                  while ($row = mysql_fetch_array($sql)) {
                      echo "<tr>";
                      echo "<td colspan=2><a href=index.php?showrev=$show_data_for_rev&filterstatus=$filter_status&quest=" . $row["entry"] . ">" . $row["entry"] . " - " . mysql_result(mysql_query("SELECT LogTitle FROM $trinitydb.quest_template WHERE ID =" . $row["entry"]), 0) . "</a></td>";
                      echo "<td colspan=2>";
              ...
              */
              elseif (isset($_GET["recent"])) {
                  ...
                  while ($row = mysql_fetch_array($sql)) {
                      echo "<tr>";
                      echo "<td><a href=index.php?showrev=$show_data_for_rev&quest=" . $row["quest_id"] . ">" . $row["quest_id"] . " - " . mysql_result(mysql_query("SELECT LogTitle FROM $trinitydb.quest_template WHERE ID =" . $row["quest_id"]), 0) . "</a></td>";
              ...
              */
              elseif ($map == "x" && $areasort == "x" && $quest == "x") {
                  ...
                  $list = Array();
                  //First a total count
                  $all = mysql_fetch_assoc(mysql_query("SELECT COUNT(ID) AS num,group_concat(DISTINCT QuestSortID SEPARATOR \",\") AS zones FROM $trinitydb.quest_template"));
                  $list[] = Array("num"=>$all["num"], "zones"=>$all["zones"], "map"=>"x", "name"=>"All Quests", "type"=>-1);

                  //Regular Maps, Instances, Raids, BGs
                  $sql = mysql_query("SELECT count( mq.ID ) AS num, group_concat(DISTINCT mq.QuestSortID SEPARATOR \",\") AS zones, ta.map, tm.name, tm.type FROM $trinitydb.quest_template AS mq, $trackerdb.areatable AS ta, $trackerdb.map AS tm WHERE mq.QuestSortID >0 AND mq.QuestSortID = ta.id AND ta.map = tm.id GROUP BY tm.id ASC ORDER BY tm.type, ta.map ASC");
                  while ($row = mysql_fetch_assoc($sql))
                  {
                      $list[] = $row;
                  }
                  //Profession, Class and Event quests need custom grouping
                  $professions_count = mysql_result(mysql_query("SELECT COUNT(ID) AS num FROM $trinitydb.quest_template WHERE QuestSortID IN (" . $zos_profession . ")"),0);
                  $list[] = Array("num"=>$professions_count, "zones"=>$zos_profession, "map"=>"p", "name"=>"Profession", "type"=>999);
                  $class_count = mysql_result(mysql_query("SELECT COUNT(ID) AS num FROM $trinitydb.quest_template WHERE QuestSortID IN (" . $zos_class . ")"),0);
                  $list[] = Array("num"=>$class_count, "zones"=>$zos_class, "map"=>"c", "name"=>"Class", "type"=>999);
                  $event = mysql_fetch_assoc(mysql_query("SELECT COUNT(ID) AS num,group_concat(DISTINCT QuestSortID SEPARATOR \",\") AS zones FROM $trinitydb.quest_template WHERE QuestSortID <0 AND QuestSortID NOT IN (" . $zos_class . "," . $zos_profession . ")"),0);
                  $list[] = Array("num"=>$event["num"], "zones"=>$event["zones"], "map"=>"e", "name"=>"Event", "type"=>999);
                  $other_count = mysql_result(mysql_query("SELECT COUNT(ID) AS num FROM $trinitydb.quest_template WHERE QuestSortID = 0"),0);
                  ...
                  for ($i =0; $i < count($list); $i++)
                  {
                      ...
                      $unknown = $line["num"] - mysql_result(mysql_query("SELECT COUNT(DISTINCT quest_id) FROM $trackerdb.status WHERE quest_id IN (SELECT ID FROM $trinitydb.quest_template WHERE QuestSortID IN (".$line["zones"].")) AND status > 0 AND (dbver=" . (is_numeric($show_data_for_rev) ? $show_data_for_rev : "0 or dbver>0") . ")"), 0);
                      $track_status = "";
                      $working = array();
                      for ($j = 1; $j < count($status); $j++) {
                          $working[$j] = mysql_result(mysql_query("SELECT COUNT(DISTINCT quest_id) FROM $trackerdb.status WHERE quest_id in (SELECT ID FROM $trinitydb.quest_template WHERE QuestSortID IN (".$line["zones"].")) AND status = $j AND (dbver=" . (is_numeric($show_data_for_rev) ? $show_data_for_rev : "0 or dbver>0") . ")"), 0);
              ...
              if ($map !== "x" && $areasort === "x" && $quest == "x") {
                  if (is_numeric($map)) {
                      ...
                      $selector = "Zone";
                      $sql = mysql_query("SELECT COUNT(m.ID) AS num, m.QuestSortID,t.id, t.name FROM $trinitydb.quest_template as m, $trackerdb.areatable as t WHERE t.map=$map AND t.id = m.QuestSortID GROUP BY m.QuestSortID ASC") or die(mysql_error());
                  } elseif ($map == "p") {
                       //professions
                      $mapname = "Professions";
                      $selector = "Profession";
                      $sql = mysql_query("SELECT COUNT(m.ID) AS num, m.QuestSortID,t.id, t.name FROM $trinitydb.quest_template as m, $trackerdb.questsort as t WHERE t.id = -1*m.QuestSortID AND m.QuestSortID IN (".$zos_profession.") GROUP BY m.QuestSortID ASC") or die(mysql_error());
                  } elseif ($map == "c") {
                       //classes
                      $mapname = "Classes";
                      $selector = "Class";
                      $sql = mysql_query("SELECT COUNT(m.ID) AS num, m.QuestSortID,t.id, t.name FROM $trinitydb.quest_template as m, $trackerdb.questsort as t WHERE t.id = -1*m.QuestSortID AND m.QuestSortID IN (" . $zos_class . ") GROUP BY m.QuestSortID ASC") or die(mysql_error());
                  } elseif ($map == "e") {
                       //events
                      $mapname = "Events";
                      $selector = "Event";
                      $sql = mysql_query("SELECT COUNT(m.ID) AS num, m.QuestSortID,t.id, t.name FROM $trinitydb.quest_template as m, $trackerdb.questsort as t WHERE t.id = -1*m.QuestSortID AND m.QuestSortID NOT IN (" . $zos_class . "," . $zos_profession . ") GROUP BY m.QuestSortID ASC") or die(mysql_error());
                  } elseif ($map == "u") {
                  ...
                  while ($row = mysql_fetch_assoc($sql)) {//
                      $unknown = $row["num"] - mysql_result(mysql_query("SELECT COUNT(DISTINCT quest_id) FROM $trackerdb.status WHERE quest_id IN (SELECT ID FROM $trinitydb.quest_template WHERE QuestSortID = ".$row["QuestSortID"].") AND status > 0 AND (dbver=" . (is_numeric($show_data_for_rev) ? $show_data_for_rev : "0 or dbver>0") . ")"), 0);
                      $track_status = "";
                      $working = array();
                      for ($j = 1; $j < count($status); $j++) {
                          $working[$j] = mysql_result(mysql_query("SELECT COUNT(DISTINCT quest_id) FROM $trackerdb.status WHERE quest_id in (SELECT ID FROM $trinitydb.quest_template WHERE QuestSortID = ".$row["QuestSortID"].") AND status = $j AND (dbver=" . (is_numeric($show_data_for_rev) ? $show_data_for_rev : "0 or dbver>0") . ")"), 0);
                          if ($working[$j] != 0 && (!is_numeric($filter_status) || $filter_status==$j))
                              $track_status .="/<font color=" . $statuscolor[$j] . ">" . $working[$j] . "</font>";
                      }
                      if(is_numeric($filter_status) && empty($track_status))
              ...
              elseif (is_numeric($areasort)) {
                  ...
                  $sql = mysql_query("SELECT m.ID, m.LogTitle, m.RequiredRaces, m.QuestLevel FROM $trinitydb.quest_template as m WHERE m.QuestSortID =$areasort") or die(mysql_error());
